...
// CRUD Operations
	...
	@DeleteMapping("/{id}") // Handles DELETE request to /api/employees/{id}
    public ResponseEntity<Void> deleteEmployee(@PathVariable Long id) { 
        employeeService.deleteEmployee(id); // Call service to delete employee by ID
        return ResponseEntity.noContent().build(); // Return HTTP 204 No Content (successful deletion)
    }
}
