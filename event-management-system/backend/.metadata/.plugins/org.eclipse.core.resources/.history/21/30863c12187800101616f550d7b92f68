...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
            .cors(cors -> cors.configurationSource(corsConfigurationSource())) // ✅ CORRECT CORS PLACEMENT
            .csrf(csrf -> csrf.disable())
            .authorizeHttpRequests(auth -> auth
            		.requestMatchers("/api/auth/**").permitAll()
            		.requestMatchers("/api/events/**").hasAnyRole("ADMIN", "ATTENDEE")
            		.requestMatchers("/api/rsvps/event/**").hasRole("ADMIN")
            		.requestMatchers("/api/rsvps/user/**").hasRole("ADMIN")
            		.requestMatchers("/api/rsvps/user").hasRole("ATTENDEE")
            		.requestMatchers(HttpMethod.GET, "/api/rsvps").hasRole("ADMIN") // GET all RSVPs (admin only)
            		.requestMatchers(HttpMethod.POST, "/api/rsvps").hasRole("ATTENDEE") // POST RSVP (attendee only)
            		.requestMatchers("/api/rsvps/my-rsvps/**").hasRole("ATTENDEE")
            		.anyRequest().authenticated()
            // Specific order to avoid duplicates
            )
